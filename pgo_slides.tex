...
\frametitle{Random-number generators}
  \toplink{http://en.cppreference.com/w/cpp/numeric/random}

\begin{itemize}
  \item New \texttt{random} header provides several pseudorandom-number
        generators and
        20 commonly used distributions, including many not implemented in
        ROOT, such as Bernoulli, negative-binomial, $\gamma$, etc.
...
\begin{itemize}
  \item See also \texttt{std::shared\_ptr} for shared ownership.
\end{itemize}
...
\begin{itemize}
  \item Combine with \texttt{std::unique\_ptr} for safe, portable dynamic
        memory.
\end{itemize}
